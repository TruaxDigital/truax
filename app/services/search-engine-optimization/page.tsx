      ...
      <section className="py-20 px-6 border-t border-[#262466]/50">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">Latest on SEO & Search</h2>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-8">
            {getPostsByKeywords(["seo", "search engine", "google", "ranking", "keywords", "organic", "ai overview"], 3).map((post, index) => (
              <motion.article
                key={post.slug}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {(() => {
                      ...
                      return imageUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <SearchIcon className="h-10 w-10 text-[#27AAE1]/40" />
                        </div>
                      );
                    })()}
                  </div>
